...
int NetworkAgent::set_on_server_connected_fn(std::nullptr_t)
{
    return set_on_server_connected_fn(AppOnServerConnectedFn());
}
...
int NetworkAgent::set_on_server_connected_fn(OnServerConnectedFn fn)
{
    if (!fn)
        return set_on_server_connected_fn(AppOnServerConnectedFn());
    return set_on_server_connected_fn([fn](CloudEvent, int return_code, int reason_code) {
        fn(return_code, reason_code);
    });
}

int NetworkAgent::set_on_http_error_fn(std::nullptr_t)
{
    return set_on_http_error_fn(AppOnHttpErrorFn());
}
...
int NetworkAgent::set_on_http_error_fn(OnHttpErrorFn fn)
{
    if (!fn)
        return set_on_http_error_fn(AppOnHttpErrorFn());
    return set_on_http_error_fn([fn](CloudEvent, unsigned http_code, std::string http_body) {
        fn(http_code, std::move(http_body));
    });
}
...
{
    int result = 0;
    for (const auto& cloud_agent_pair : m_cloud_agents) {
        if (auto orca_agent = std::dynamic_pointer_cast<OrcaCloudServiceAgent>(cloud_agent_pair.second)) {
            const int ret = orca_agent->set_on_user_login_fn(fn);
            if (result == 0 && ret != 0)
                result = ret;
        }
    }
...
int NetworkAgent::set_extra_http_header(std::map<std::string, std::string> headers)
{
    int result = 0;
    for (const auto& cloud_agent_pair : m_cloud_agents) {
        if (auto orca_agent = std::dynamic_pointer_cast<OrcaCloudServiceAgent>(cloud_agent_pair.second)) {
            const int ret = orca_agent->set_extra_http_header(headers);
            if (result == 0 && ret != 0)
                result = ret;
        }
    }
